...
let battlefield = [
   ...
   // ,// polygon #4
    ...
    , new Solder({ x: canvas.width - 50, y: canvas.height *4/ 5 }, 'red', canvas,3)
    ...
    , new Solder({ x: 50, y: canvas.height * 4 / 5 }, 'green',canvas,3)
];
...
leftclickListener = function (e) {
    ...
    for (let unit of units) {
        let bullets = unit.fire({ x: event.clientX, y: event.clientY });
        if (bullets != null) {
            for (let i = 0; i < bullets.length; i++) {
                battlefield.push(bullets[i]);
            }
        }
    }
...
mouseMoveListener = function(e){
...
else
    {
        ...
        let units = battlefield.filter((d) => d.iskey('solder') && d.hasSelect());
        //units.forEach((d) => d.SAttack = point); 
    }
}
let ks= [];
...
keyUpListener = function (e) {
    ks[e.keyCode] = false;
    //let units = battlefield.filter((d) => d.iskey('solder') && d.hasSelect());
    //ssasunits.forEach((d) => d.control(ks, battlefield));
}
